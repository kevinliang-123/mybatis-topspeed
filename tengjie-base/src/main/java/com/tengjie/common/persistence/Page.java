...
import com.tengjie.common.utils.CookieUtils;
...
public class Page<T>implements Serializable   {
	...
	protected int pageSize =StringUtils.isEmpty(Global.getConfig("page.pageSize"))?10:Integer.valueOf(Global.getConfig("page.pageSize")); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	 ...
	 */
	public Page(T queryParam){
		// 设置页码参数（传递repage参数，来记住页码）
		if(!(queryParam instanceof TjBaseEntity)){
			System.out.println("不是BaseEntity类型，可能无法找到pageNo属性");
		}
				Integer no = ((TjBaseEntity)queryParam).getPageNo();
				if(no>0)
				this.setPageNo(no);
				Integer size =((TjBaseEntity)queryParam).getPageSize();
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNumeric(no)){
			//此处存储两个pageNo，一个是带有list，一个是不带list的，主要是因为下面的repage参数，如果在更新页面或其他地方（不是第一页）进行删除后，
			//返回的url使用了?repage，由于这里存储了pageNo，那么直接会使用这个，但是第一个记录的是uri/listpageno的cookie的key，
			//而标准写法是用/list?repage，但是很多人都不写这个，因此这里存储两个，一个带list，一个不带list；但是注意，如果一个页面有多个list页面的时候
			//即调用findpage那种写法，还是要标准的带上list方法名的
			String removeListUrl=request.getRequestURI().substring(0,request.getRequestURI().lastIndexOf("/")).replace("/", "");
			CookieUtils.setCookie(response, request.getRequestURI().replace("/", "")+"pageNo", no);
			CookieUtils.setCookie(response, removeListUrl+"pageNo", no); 
			this.setPageNo(Integer.parseInt(no));
		}else if (request.getParameter("repage")!=null){
			no = CookieUtils.getCookie(request, request.getRequestURI().replace("/", "")+"pageNo");
			if (StringUtils.isNumeric(no)){
				this.setPageNo(Integer.parseInt(no));
			}
		}
