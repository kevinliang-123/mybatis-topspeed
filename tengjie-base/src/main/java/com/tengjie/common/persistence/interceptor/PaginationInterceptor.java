...
//        }
          //map分页
            ...
            if (parameterObject != null) {
                pageMap = convertParameter(parameterObject);
            }
            if (pageMap != null && pageMap.getPageSize() != -1) {
                if (StringUtils.isBlank(boundSql.getSql())){
                    return null;
                }
                String sql = boundSql.getSql().trim();
                //得到总记录数
                if(pageMap.getIfCount()){
                    pageMap.setCount(SQLHelper.getCount(sql, null, mappedStatement, parameterObject, boundSql, log));
                }else{
                	pageMap.setCount(0);
                }
                pageMap.init();
                //分页查询 本地化对象 修改数据库注意修改实现
                String pageSQL = SQLHelper.generatePageSql(sql, pageMap, DIALECT);
                invocation.getArgs()[2] = new RowBounds(RowBounds.NO_ROW_OFFSET, RowBounds.NO_ROW_LIMIT);
                BoundSql newBoundSql = new BoundSql(mappedStatement.getConfiguration(), pageSQL, boundSql.getParameterMappings(), boundSql.getParameterObject());
                if (Reflections.getFieldValue(boundSql, "metaParameters") != null) {
                    MetaObject mo = (MetaObject) Reflections.getFieldValue(boundSql, "metaParameters");
